...
function readData() {
    ...
    .then(res => res.json())
    .then(data => {
        let trtd = data.cdeData.reverse().map(each => {
...
function delData(customerId) {
    fetch(api + `?del=true&customerId=${customerId}`)
    .then(() => {
        readData();
        form.reset();
        return showDelSuccessModal();    });
}
...
function searchGenData() {
    ...
    tableRows.forEach(row => {
        ...
        if (tin.includes(searchQuery) || name.includes(searchQuery) || address.includes(searchQuery) || registration.includes(searchQuery) || contact.includes(searchQuery) || date.includes(searchQuery)) {
            row.style.display = '';
        } else {
